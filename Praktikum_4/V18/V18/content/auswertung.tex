...
Aus dem breits experimentell bestimmten Comptonpeak kann num mit \autoref{eq:Comptonpeak} und \autoref{eq:Rückstreupeak} %Formel in der Theorie einfügen 
...
Daraus ergibt sich
\begin{equation*}
    A = \frac{Z}{Q \cdot W \cdot t \cdot \Omega}\,.
\end{equation*}
...
\begin{table}
    \centering
    \caption{Kanalnummer, Energie, Emissionswahrscheinlichkeit $W$, Spektrallinieninhalt $Z$ und der Detektoreffizienz $Q$ von Co-60.}
    \label{tab:Co60}
    \begin{tabular}{c S[table-format=4.3(1), separate-uncertainty] S[table-format=3.4(1)] S[table-format=3.0(3)] S[table-format=2.2(4)]}
        \toprule
        Kanalnummer & {$E \mathbin{/} \si{\kilo\eV}$} & {$W \mathbin{/} \%$} & {Linieninhalt} & {$Q \mathbin{/} \%$ }\\
        \midrule           
