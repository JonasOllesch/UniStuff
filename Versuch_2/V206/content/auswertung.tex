...
In der folgenden Tabelle seien die Parameter $A, B, C$ der Näherung für $T_1(t), T_2(t)$ dargestellt.

\begin{table}[H]
  \centering
  \label{tab:ApproxTemp}
  \sisetup{table-format=1.5}
  \begin{tabular}{S S S S[table-format=2.5]}
    \toprule
    & {$A \mathbin{/} \unit{\dfrac{\celsius}{\minute^2}}$} & {$B \mathbin{/} \unit{\dfrac{\celsius}{\minute}}$} & {$C \mathbin{/} \unit{\celsius}$} \\
    \midrule
    {$T_1$} & {-0.01756} & 1.50604 & 20.45657 \\
    {$T_2$} & 0.01510 & {-1.29451} & 22.77799 \\
    \bottomrule
  \end{tabular}
\end{table}
